...
function EnterPointsScreen({ setScreen, addSlip, theme, staffList, onLogout, currentUser, isMobile }) {
    ...
    const handleSubmit = (e) => {
        ...
        if (day === 0 || day === 6) {
            alert('Weekends are excluded. Please select a weekday.');
            return;
        }
...
function ViewPointsScreen({ setScreen, pointsSlips, theme, onLogout, currentUser, isMobile }) {
    ...
    const weeklySlips = pointsSlips.filter(p => p.date >= oneWeekAgo && !isWeekend(p.date));
    ...
    let daysAdded = 0;
    let offset = -dateRange;
    
    // Collect non-weekend dates
    while (dates.length < (dateRange * 2 + 1)) {
        ...
        offset++;
        if (offset > 50) break; // Safety break
    }

    useEffect(() => {
        if (activeTab === 'table' && tableContainerRef.current) {
            const container = tableContainerRef.current;
            const todayIndex = dates.findIndex(d => d.toDateString() === todayLocal.toDateString());
            const scrollAmount = (todayIndex !== -1 ? todayIndex : dateRange) * 160; 
    ...
    while (last14Days.length < 14) {
        ...
        dOffset++;
        if (dOffset > 100) break; // Safety break
    }
    last14Days.reverse();
